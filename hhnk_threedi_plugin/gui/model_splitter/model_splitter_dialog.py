...
from hhnk_threedi_plugin.tasks import checkSchematisationTask, generalChecksTask
...
class modelSplitterDialog(QtWidgets.QDialog):
    ...
    def init_widgets(self):
        ...
        self.modelschematisations = htt.model_splitter.ModelSchematisations(
            folder=self.caller.fenv, modelsettings_path=self.settings_path
        )
    ...
    def verify_upload(self, verbose=True):
        ...
        if cont and self.sql_error:
            message = (
                "Model contains errors in sqlite database and cannot be uploaded. Run sqlite checks and fix errors."
            )
            cont = False
    ...
    def create_schematisations(self):
        ...
        response = self.modelschematisations.create_local_sqlite_revision(
            commit_message=str(" (local split revision)")
        )
    ...
    def revision_check(self):
        ...
        for list_name in self.enabled_lst:
            self.info_list.addItem(
                self.modelschematisations.get_model_revision_info(name=list_name, api_key=self.api_key)
            )
    ...
    def upload_schematisations(self):
        ...
        try:
            ...
            # settings for upload
            organisations = upload.threedi.api.contracts_list(
                organisation__name=self.dockwidget.org_name_comboBox.currentText()
            ).results
            ...
            for list_name in self.enabled_lst:
                ...
                self.modelschematisations.upload_schematisation(
                    name=list_name, commit_message=commit_message, api_key=self.api_key, organisation_uuid=uuid_slug
                )
            ...
            response = self.modelschematisations.create_local_sqlite_revision(
                commit_message=("upload revision " + commit_message)
            )
