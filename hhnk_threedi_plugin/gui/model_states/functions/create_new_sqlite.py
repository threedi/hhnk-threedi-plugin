...
sys.path.append(str(Path(os.getcwd()).parent.parent.parent.parent))
...
overwrite=True
def create_schematisation(name, row):
    # settings_df = pd.read_excel(folder.model.settings.path)
    # settings_df.set_index('name',inplace=True)

    # print(settings_df.head())
    # print('name', type(name))
    # # row = settings_df.loc[name]
    schema_name = folder.model.add_modelpath(name)
     ...
     #       row = settings_df.loc[name]
      ...
      #      print (name)       
       #     print(f"Create {row['name']}") 
        #    schema_name = folder.model.add_modelpath(name,['name'])
         #   print (schema_name)
