...
from hhnk_threedi_plugin.gui.tests.sqlite_test_widgets.geometries_result import (
...
class geometriesTask(QgsTask):
    ...
    def run(self):
        ...
        try:
            ...
            self.csv_path = hrt.gdf_write_to_csv(
                ...
                filename=self.test_env.output_vars["geometry_filename"],
            )
            return True
        except OSError as e:
            self.os_retry = None
    ...
    def finished(self, result):
        ...
        if not result:
            ...
            try:
                title, widget = create_geometries_result_widget(csv_path=self.csv_path)
