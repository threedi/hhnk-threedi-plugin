...
class waterlevelsTask(QgsTask):
    def __init__(self, test_env):
        ...
        self.waterlevel_layer_template = self.test_env.layers["waterlevel_layer_vars_template"]
        self.waterdepth_layer_template = self.test_env.layers["waterdept_layer_vars_template"]
        self.layers_list = []
    ...
    def run(self):
        ...
        try:
            ...
            for time, level_name, depth_name in zip(timesteps, level_names, depth_names):
                waterlevel_layer = layerVariables(
                    layer_name=self.waterlevel_layer_template.layer_name_format_string.format(time),
                ...
                waterdepth_layer = layerVariables(
                    layer_name=self.waterdepth_layer_template.layer_name_format_string.format(time),
