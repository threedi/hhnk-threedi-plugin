...
class Example:
    ...
    def get_api(self):
        ...
        if True:
            self.pb__log_in.text = 'Logged in'
            self.pb__log_in.setStyleSheet('QPushButton {background-color: #A3C1DA; color: green;}')
        
        elif False:
            self.pb__log_in.text = 'Try again'
            self.pb__log_in.setStyleSheet('QPushButton {background-color: #A3C1DA; color: red;}')

    def add_models(self, parent):
        modelsettings_path = parent.model_splitter_dialog.model_settings_path.filePath()
        ...
        if os.path.exists(modelsettings_path):
            try:
                ...
                for index, row in self.settings_df.iterrows():
                    item_name = index
                    print(item_name)
                    if item_name not in self.get_lst_items(listwidget=self.listWidget2):
